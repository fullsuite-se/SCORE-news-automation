...
async function esgpostInvestmentScraper() {
  const browser = await puppeteer.launch({ headless: false, defaultViewport: null, slowMo: 50 });
  const page = await browser.newPage();
  const url = 'https://esgpost.com/category/esg-investment/';

  try {
    console.log('Navigating to ESGPost ESG Investment page...');
    ...
    const filename = 'esgpostInvestment.json';
