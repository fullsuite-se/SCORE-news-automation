...
export default async function (req, res) {
  ...
  let browser;
  const baseUrl = 'https://www.efrag.org/en/news-and-calendar/news';
  ...
  try {
    ...
    const articles = await page.evaluate(() => {
      const items = document.querySelectorAll('div.views-row');
      const seen = new Set();
      const results = [];

      items.forEach(item => {
        const titleEl = item.querySelector('h3.node--teaser__title a');
