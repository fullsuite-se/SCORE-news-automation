...
async function esgpostCorporateGovernanceScraper() {
  const browser = await puppeteer.launch({ headless: false, defaultViewport: null, slowMo: 50 });
  const page = await browser.newPage();
  const url = 'https://esgpost.com/category/corporate-governance/';

  try {
    console.log('Navigating to ESGPost Corporate Governance page...');
    ...
    const filename = 'esgpostCorporateGovernance.json';
...
esgpostCorporateGovernanceScraper();
