...
async function esgpostClimateScraper() {
  const browser = await puppeteer.launch({ headless: false, defaultViewport: null, slowMo: 50 });
  const page = await browser.newPage();
  const url = 'https://esgpost.com/category/climate-change/';

  try {
    console.log('Navigating to ESGPost Climate Change page...');
    ...
    const filename = 'esgpostClimate.json';
