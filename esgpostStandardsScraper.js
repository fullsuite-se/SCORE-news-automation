...
async function esgpostStandardsScraper() {
  const browser = await puppeteer.launch({ headless: false, defaultViewport: null, slowMo: 50 });
  const page = await browser.newPage();
  const url = 'https://esgpost.com/category/standards/';

  try {
    console.log('Navigating to ESGPost Standards page...');
    ...
    const filename = 'esgpostStandards.json';
