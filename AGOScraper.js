...
export default async function (req, res) {
  ...
  try {
    ...
    await page.waitForSelector('article.tileItem', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const nodes = document.querySelectorAll('article.tileItem');
      const seen = new Set();
      const results = [];

      nodes.forEach(node => {
        const anchor = node.querySelector('h2.tileHeadline a.summary.url');
        // Find the span that contains a date (looks like "DD/MM/YYYY") but not a time
        const dateSpan = Array.from(node.querySelectorAll('span.summary-view-icon'))
          .find(span => span.innerText.includes('/') && !span.innerText.includes(':'));
