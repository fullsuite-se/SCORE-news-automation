...
export default async function handler(req, res) {
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
      ...
      const articleElements = document.querySelectorAll('div.PageList-items > div.PageList-items-item');
      ...
      for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
          ...
          const titleElement = articleElement.querySelector('h3.PagePromo-title > a > span');
          ...
          // Extract Link
          // <--- REPLACE THIS SELECTOR
          const linkElement = articleElement.querySelector('h3.PagePromo-title > a');
