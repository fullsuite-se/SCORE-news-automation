...
export default async function (req, res) {
  ...
  let browser;
  const baseUrl = 'https://www.news.admin.ch/de/newnsb?publisherIDs=89&topicIDs=399&sort=dateDecreasing&display=list&start_date=2024-06-24&end_date=2025-06-24&newsCategoryIDs=all';
  ...
  try {
    ...
    const articles = await page.evaluate(() => {
      ...
      const results = [];
      const seenUrls = new Set(); // To prevent duplicates
      ...
      newsItems.forEach(item => {
        ...
        const titleEl = item.querySelector('.card__title h2');
        if (titleEl) {
          title = titleEl.textContent.trim();
        }

        const urlEl = item.querySelector('.card__footer__action a.btn');
        if (urlEl) {
          url = new URL(urlEl.getAttribute('href'), window.location.origin).href;
        }

        const dateEl = item.querySelector('p.meta-info span.meta-info__item');
        if (dateEl) {
    ...
    console.log(`Returning ${articles.length} news items.`);
