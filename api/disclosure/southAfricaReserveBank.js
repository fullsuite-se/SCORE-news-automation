...
export default async function handler(req, res) {
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = document.querySelectorAll('div.resultItemContainer > div.resultListItem');
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            const titleElement = articleElement.querySelector('span.resultListItem__title > b');
            const title = titleElement ? titleElement.innerText.trim() : 'N/A';

            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('div.resultListItem__content > span:nth-child(2)');
