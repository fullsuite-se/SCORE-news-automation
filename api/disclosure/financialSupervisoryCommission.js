...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.fsc.gov.tw/en/home.jsp?id=54&parentpath=0%2C2&mcustomize=multimessage_list.jsp';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = Array.from(document.querySelectorAll('div.newslist > ul > li')).slice(1);
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('span.date:nth-child(2)');
