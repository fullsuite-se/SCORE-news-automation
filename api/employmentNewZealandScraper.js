...
export default async function (req, res) {
  ...
  let browser;
  const baseUrl = 'https://www.employment.govt.nz/news-and-updates';
  ...
  try {
    ...
    await page.waitForSelector('li.py-6', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const items = document.querySelectorAll('li.py-6 div.flex.flex-col.gap-4.max-w-content');
      const seen = new Set();
      const results = [];

      items.forEach(item => {
        const link = item.querySelector('a.h5.block');
        const title = link ? link.textContent.trim() : null;
        const href = link ? link.href : null;
        const dateEl = item.querySelector('p.h6 time');
        const dateRaw = dateEl ? dateEl.getAttribute('datetime').trim() : null;
        let date = null;

        if (dateRaw) {
          date = dateRaw.replace(/(T| )00:00:00.*$/, '');
        }

        if (title && href && date && !seen.has(href)) {
          seen.add(href);
