...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://english.www.gov.cn/policies/latestreleases';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
            ...
            const articleElements = document.querySelectorAll('div.TopNews_List_Div > ul > li');
            ...
            for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
                ...
                // Extract Date
                // <--- REPLACE THIS SELECTOR
                const dateElement = articleElement.querySelector('h4');
