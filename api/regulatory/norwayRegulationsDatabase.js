...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://lovdata.no/artikler';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = document.querySelectorAll('div.news-items > article');
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('p.meta > span.time');
            // const date = dateElement ? dateElement.innerText.trim() : 'N/A'
            const date = dateElement ? dateElement.innerText.replace(/\n/g, ' ').trim() : 'N/A';

            // Extract Link
            // <--- REPLACE THIS SELECTOR
            const linkElement = articleElement.querySelector('h2 > a');
