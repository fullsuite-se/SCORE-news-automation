...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.dhs.gov/archive/news?combine=UFLPA&created=&field_news_type_target_id=All&items_per_page=10';
  ...
  try {
   ...
   const scrapedData = await page.evaluate((maxArticles) => {
            ...
            const articleElements = document.querySelectorAll('ul.usa-collection > li.usa-collection__item');
            ...
            for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
                ...
                const titleElement = articleElement.querySelector('h3.usa-collection__heading > a');
                const title = titleElement ? titleElement.innerText.trim() : 'N/A';

                // Extract Date
                // <--- REPLACE THIS SELECTOR
                // const dateElement = articleElement.querySelector('span.usa-collectyion__calendar-date-month');
                // const date = dateElement ? dateElement.innerText.trim() : 'N/A'

                const monthDayElement = articleElement.querySelector('span.usa-collection__calendar-date-month'); // <--- REPLACE THIS SELECTOR
                const yearElement = articleElement.querySelector('span.usa-collection__calendar-date-day');     // <--- REPLACE THIS SELECTOR

                let extractedDate = 'N/A';
                if (monthDayElement && yearElement) {
                    extractedDate = `${monthDayElement.innerText.trim()} ${yearElement.innerText.trim()}`;
                } else if (monthDayElement) { // Fallback if only month/day is available
                    extractedDate = monthDayElement.innerText.trim();
                } else if (yearElement) { // Fallback if only year is available
                    extractedDate = yearElement.innerText.trim();
                }
                // If there's a time element with datetime attribute, you might still prefer that
                const timeElement = articleElement ? articleElement.querySelector('time.gsc-date__date') : null;
                const timeFromDatetime = timeElement ? timeElement.getAttribute('datetime') : '';

                // Prioritize datetime attribute, then combined month/day/year, then groupDate
                const date = timeFromDatetime || extractedDate || currentGroupDate;

                // Extract Link
                // <--- REPLACE THIS SELECTOR
                const linkElement = articleElement.querySelector('h3.usa-collection__heading > a');
