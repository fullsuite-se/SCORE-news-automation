...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://kemenlh.go.id/news/main/siaran_pers';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
            ...
            const articleElements = document.querySelectorAll('#newsListContainer > div.col-lg-4');
            ...
            for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
                ...
                const titleElement = articleElement.querySelector('a > div.card > div.card-body > h5.card-title');
                const title = titleElement ? titleElement.innerText.trim() : 'N/A';

                // Extract Date
                // <--- REPLACE THIS SELECTOR
                const dateElement = articleElement.querySelector('a > div.card > div.card-body > p.card-text');
