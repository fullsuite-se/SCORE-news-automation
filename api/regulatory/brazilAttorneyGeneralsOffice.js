...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.gov.br/agu/pt-br/comunicacao/noticias';
  ...
  try {
    ...
    const acceptButtonSelector = 'button.btn-accept'; // Example: A button with ID 'accept-cookies'
        const cookieBannerSelector = 'div.br-cookiebar'; // Example: The banner div itself

        console.log("Checking for cookie consent banner...");
        try {
            // Wait for the cookie accept button to appear and be clickable (visible and not disabled)
            await page.waitForSelector(acceptButtonSelector, { visible: true, timeout: 5000 });
            console.log("Cookie accept button found. Attempting to click...");

            // Use page.evaluate to click the element directly via JavaScript, which is often more robust
            ...
            console.log("Cookie accept button clicked (via evaluate).");

            // Wait for the cookie banner to disappear after clicking (optional, but robust)
            // This ensures the page is ready for scraping without the banner obstructing elements.
            await page.waitForSelector(cookieBannerSelector, { hidden: true, timeout: 5000 });
            console.log("Cookie banner disappeared.");

        } catch (cookieError) {
            console.warn(`No cookie banner/accept button found or it timed out, or click failed. Proceeding without explicit cookie acceptance. Error: ${cookieError.message}`);
            // This is okay if the site doesn't have a banner or it's already dismissed.
        }
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
            ...
            const articleElements = document.querySelectorAll('div#content-core > article.tileItem.visualIEFloatFix.tile-collective-nitf-content');
            ...
            for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
                ...
                const titleElement = articleElement.querySelector('h2.tileHeadline > a.summary.url');
                const title = titleElement ? titleElement.innerText.trim() : 'N/A';

                // Extract Date
                // <--- REPLACE THIS SELECTOR
                const dateElement = articleElement.querySelector('span.documentByLine > span.summary-view-icon:has(i.icon-day)');
                const date = dateElement ? dateElement.innerText.trim() : 'N/A'

                // Extract Link
                // <--- REPLACE THIS SELECTOR
                const linkElement = articleElement.querySelector('h2.tileHeadline > a.summary.url');
