...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.argentina.gob.ar/capital-humano/trabajo/noticias';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
            ...
            const articleElements = document.querySelectorAll('#divnoticias > div.row > div.col-xs-12');
            ...
            for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
                ...
                const titleElement = articleElement.querySelector('a > div.panel-body > h3');
                const title = titleElement ? titleElement.innerText.trim() : 'N/A';

                // Extract Date
                // <--- REPLACE THIS SELECTOR
                const dateElement = articleElement.querySelector('a > div.panel-body > time');
