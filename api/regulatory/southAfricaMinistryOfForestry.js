...
export default async function (req, res) {
  ...
  const url = `https://www.dffe.gov.za/index.php/media-releases`
  const articles = [];
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
            ...
            const articleElements = document.querySelectorAll('table.cols-2 tbody tr');
            ...
            for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
                ...
                // Extract Date
                // <--- REPLACE THIS SELECTOR
                const dateElement = articleElement.querySelector('td:nth-child(1)');
