...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.employment.govt.nz/news-and-updates';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = document.querySelectorAll('ul.flex.flex-col > li.py-6');
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            const titleElement = articleElement.querySelector('div.flex.flex-col > a');
            const title = titleElement ? titleElement.innerText.trim() : 'N/A';
            
            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('div.flex.flex-col > p.h6 > time');
            // const date = dateElement ? dateElement.innerText.trim() : 'N/A'
            const date = dateElement ? dateElement.innerText.replace(/\n/g, ' ').trim() : 'N/A';

            // Extract Link
            // <--- REPLACE THIS SELECTOR
            const linkElement = articleElement.querySelector('div.flex.flex-col > a');
