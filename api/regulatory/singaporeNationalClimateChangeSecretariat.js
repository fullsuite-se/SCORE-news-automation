...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.nccs.gov.sg/media/press-releases/';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = document.querySelectorAll('div.is-multiline > div.resource-card-element');
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            const titleElement = articleElement.querySelector('a > div.media-card-plain > div > h5.has-text-white > b');
            const title = titleElement ? titleElement.innerText.trim() : 'N/A';
            
            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('a > div.media-card-plain > div.description > small');
