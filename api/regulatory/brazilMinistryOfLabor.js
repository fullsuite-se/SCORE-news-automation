...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.gov.br/trabalho-e-emprego/pt-br/noticias-e-conteudo';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
            ...
            const articleElements = document.querySelectorAll('#ultimas-noticias > ul.noticias > li');
            ...
            for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
                ...
                const titleElement = articleElement.querySelector('div.conteudo > h2.titulo > a');
                const title = titleElement ? titleElement.innerText.trim() : 'N/A';

                // Extract Date
                // <--- REPLACE THIS SELECTOR
                const dateElement = articleElement.querySelector('div.conteudo > span.descricao > span.data');
                const date = dateElement ? dateElement.innerText.trim() : 'N/A'

                // Extract Link
                // <--- REPLACE THIS SELECTOR
                const linkElement = articleElement.querySelector('div.conteudo > h2.titulo > a');
