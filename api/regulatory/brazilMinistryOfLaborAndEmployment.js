...
export default async function (req, res) {
  ...
  try {
    ...
    await page.waitForSelector('div.conteudo h2.titulo a', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const articleNodes = document.querySelectorAll('div.conteudo');
      const seen = new Set();
      const results = [];

      articleNodes.forEach(node => {
        const anchor = node.querySelector('h2.titulo a');
        const dateSpan = node.querySelector('span.descricao span.data');
