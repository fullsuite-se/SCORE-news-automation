...
export default async function (req, res) {
  ...
  let browser;
  const url = 'https://www.cleanenergywire.org/news#';
  ...
  try {
    ...
    await page.waitForSelector('h2.m-node--list--teaser__type', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const nodes = document.querySelectorAll('div.views-row');
      const seen = new Set();
      const results = [];

      nodes.forEach(node => {
        const linkEl = node.querySelector('h3.m-node--list--teaser__title a');
        const dateEl = node.querySelector('div.m-node--list--teaser__date_author span.date-display-single');
