...
export default async function (req, res) {
  ...
  try {
    ...
    const articles = await page.evaluate(() => {
      const blocks = document.querySelectorAll('.query-list-item-body');
      const seen = new Set();
      const results = [];

      blocks.forEach(block => {
        const linkEl = block.querySelector('h4.query-list-item-headline.wp-block-post-title a[target="_self"]');
        const dateEl = block.querySelector('div.wp-block-post-date time');

        if (linkEl && dateEl) {
          const title = linkEl.textContent.trim();
          const url = new URL(linkEl.getAttribute('href'), window.location.origin).href;
          const date = dateEl.getAttribute('datetime')?.trim() || dateEl.textContent.trim();
