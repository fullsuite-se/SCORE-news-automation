...
export default async function (req, res) {
  ...
  let browser;
  const baseUrl = 'https://www.env.go.jp/en/headline/index.html';
  ...
  try {
    ...
    await page.waitForSelector('.p-press-release-list__body', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const items = document.querySelectorAll('.p-press-release-list__body .c-news-link__item');
      const seen = new Set();
      const results = [];

      items.forEach(item => {
        const linkDivs = item.querySelectorAll('.c-news-link__item__col');
        if (linkDivs.length >= 2) {
          const link = linkDivs[1].querySelector('a.c-news-link__link');
          if (link) {
    ...
    for (let article of articles) {
      try {
        ...
        const date = await articlePage.evaluate(() => {
          const dateEl = document.querySelector('.p-press-release-material__head .p-press-release-material__date');
          return dateEl ? dateEl.textContent.trim() : 'No date found';
        });
