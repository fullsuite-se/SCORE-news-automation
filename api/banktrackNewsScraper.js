...
export default async function (req, res) {
  ...
  let browser;
  const url = 'https://www.banktrack.org/banktrack_news';
  ...
  try {
    ...
    await page.waitForSelector('div.article.item.article_article.frombanktrack', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const nodes = document.querySelectorAll('div.article.item.article_article.frombanktrack');
      const seen = new Set();
      const results = [];

      nodes.forEach(node => {
        const linkEl = node.querySelector('a[href]');
        const titleEl = node.querySelector('.title-info h1');
