...
export default async function (req, res) {
  ...
  let browser;
  const url = 'https://www.epa.gov/newsreleases/search?search_api_views_fulltext=';
  ...
  try {
    ...
    await page.waitForSelector('div.usa-collection__body', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const nodes = document.querySelectorAll('div.usa-collection__body');
      const seen = new Set();
      const results = [];

      nodes.forEach(node => {
        const anchor = node.querySelector('h3.usa-collection__heading a.usa-link');
        const time = node.querySelector('li.usa-collection__meta-item time');
