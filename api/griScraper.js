...
export default async function (req, res) {
  ...
  let browser;
  const baseUrl = 'https://www.globalreporting.org/news/news-center/';
  ...
  try {
    ...
    await page.waitForSelector('.list-card__body', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const items = document.querySelectorAll('.list-card__body');
      const seen = new Set();
      const results = [];

      items.forEach(item => {
        const titleEl = item.querySelector('h4 a');
        const dateEl = item.querySelector('.list-card__date');
