...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.oas.org/en/iachr/';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = document.querySelectorAll('div.comunicados > ul > li');
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('span');
