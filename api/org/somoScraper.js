...
export default async function (req, res) {
  ...
  let browser;
  const url = 'https://www.somo.nl/latest-updates/';
  ...
  try {
    ...
    await page.waitForSelector('div[data-component="teaser-post"]', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const nodes = document.querySelectorAll('div[data-component="teaser-post"]');
      const seen = new Set();
      const results = [];

      nodes.forEach(node => {
        const anchor = node.querySelector('a.break-words.absolute-link');
        const dateEl = node.querySelector('div.order-3 time');
