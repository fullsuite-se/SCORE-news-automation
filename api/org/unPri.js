...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.unpri.org/press-releases/30283.more?navcode=2187';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = document.querySelectorAll('div.listBlocks > ul > li');
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            const titleElement = articleElement.querySelector('div.storyDetails > h3 > a');
            const title = titleElement ? titleElement.innerText.trim() : 'N/A';
            
            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('div.storyDetails > p.meta > span.date');
            const date = dateElement ? dateElement.textContent.replace(/\s+/g, ' ').trim() : 'N/A';

            // Extract Link
            // <--- REPLACE THIS SELECTOR
            const linkElement = articleElement.querySelector('div.storyDetails > h3 > a');
