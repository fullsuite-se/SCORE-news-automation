...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.iccr.org/post-directory/';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = document.querySelectorAll('div#data-container-directory > div');
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            const titleElement = articleElement.querySelector('article.card > div.card__body > span.card__title');
            const title = titleElement ? titleElement.innerText.trim() : 'N/A';
            
            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('article.card > div.card__body > span.card__date');
            const date = dateElement ? dateElement.textContent.replace(/\s+/g, ' ').trim() : 'N/A';

            // Extract Link
            // <--- REPLACE THIS SELECTOR
            const linkElement = articleElement.querySelector('article.card > a');
