...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.leighday.co.uk/news/news/2025-news/';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = document.querySelectorAll('div.s-listings__items > div.b-card');
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            const titleElement = articleElement.querySelector('div.b-card__content > div.b-card__content__title > h3');
            const title = titleElement ? titleElement.innerText.trim() : 'N/A';
            
            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('div.b-card__footer > div.b-card__footer__published > p');
