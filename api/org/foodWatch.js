...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.foodwatch.org/en/news';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
        ...
        const articleElements = document.querySelectorAll('div.teaser-list > div.teaser-list__item');
        ...
        for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
            ...
            const titleElement = articleElement.querySelector('article > div > header > h3 > a');
            const title = titleElement ? titleElement.innerText.trim() : 'N/A';
            
            // Extract Date
            // <--- REPLACE THIS SELECTOR
            const dateElement = articleElement.querySelector('article > div > header > div > span:nth-child(2)');
            const date = dateElement ? dateElement.textContent.replace(/\s+/g, ' ').trim() : 'N/A';
    
            // Extract Link
            // <--- REPLACE THIS SELECTOR
            const linkElement = articleElement.querySelector('article > div > header > h3 > a');
