...
export default async function handler(req, res) {
  let browser; 
  const url = 'https://www.igsd.org/category/press-releases/';
  ...
  try {
    ...
    const scrapedData = await page.evaluate((maxArticles) => {
      ...
      const articleElements = document.querySelectorAll('div.col-md-12 > div.row > div.col');
      ...
      for (let i = 0; i < Math.min(articleElements.length, maxArticles); i++) {
          ...
          const titleElement = articleElement.querySelector('article > div.card__body > span.card__title > a');
          const title = titleElement ? titleElement.innerText.trim() : 'N/A';

          // Extract Date
          // <--- REPLACE THIS SELECTOR
          const dateElement = articleElement.querySelector('article > div.card__body > span.card__date');
          const date = dateElement ? dateElement.innerText.trim() : 'N/A'
          // const date = dateElement ? dateElement.getAttribute('datetime') : 'N/A'

          // Extract Link
          // <--- REPLACE THIS SELECTOR
          const linkElement = articleElement.querySelector('article > div.card__body > span.card__title > a');
