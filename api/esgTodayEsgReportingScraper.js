...
export default async function (req, res) {
  ...
  let browser;
  const baseUrl = 'https://www.esgtoday.com/category/esg-news/esg-reporting/';
  ...
  try {
    ...
    const articles = await page.evaluate(() => {
      const articleNodes = document.querySelectorAll('.post-content');
      const seen = new Set();
      const results = [];

      articleNodes.forEach(article => {
        const titleAnchor = article.querySelector('h2.post-title.entry-title > a');
        const dateEl = article.querySelector('time.post-date');

        if (titleAnchor && dateEl) {
          const title = titleAnchor.textContent.trim();
          const url = titleAnchor.href.trim();
          const date = dateEl.getAttribute('datetime').trim();
