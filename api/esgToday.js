...
export default async function (req, res) {
  ...
  let browser;
  const url = 'https://www.esgtoday.com/category/esg-news/';
  ...
  try {
    ...
    await page.waitForSelector('h2.post-title.entry-title', { timeout: 10000 });

    const articles = await page.evaluate(() => {
      const nodes = document.querySelectorAll('h2.post-title.entry-title');
      const seen = new Set();
      const results = [];
      ...
      nodes.forEach(h2 => {
        ...
        const time = parent?.querySelector('p.post-meta time.post-date');

        const title = anchor?.textContent.trim();
        const url = anchor?.href?.trim();
        const date = time?.getAttribute('datetime')?.trim();
