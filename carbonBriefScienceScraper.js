...
async function carbonBriefScienceScraper() {
  const browser = await puppeteer.launch({ headless: false, slowMo: 50 });
  const page = await browser.newPage();
  const url = 'https://www.carbonbrief.org/science/';

  try {
    console.log('Navigating to Carbon Brief Science page...');
    ...
    const articles = await page.evaluate(() => {
      ...
      const rawArticles = containers.map(container => {
        ...
        const titleTag = container.querySelector('p.text-header-xxl.title, p.text-header-xs, p.text-header-s.line-clamp');
    ...
    const filename = 'carbonBriefScience.json';
