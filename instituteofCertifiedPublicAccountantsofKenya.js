...
/**
 * Next.js API route handler for scraping ICPAK press releases.
...
export default async function handler(req, res) {
  ...
  const url = 'https://www.icpak.com/press-releases';
  ...
  try {
    ...
    // A short delay to ensure all dynamic content loads, if any.
    // In a production environment, prefer waitForSelector or networkidle0.
    await new Promise(resolve => setTimeout(resolve, 5000));

    console.log('Scraping articles...');
    // Extract article data using page.evaluate
    const articles = await page.evaluate(() => {
      const nodes = document.querySelectorAll('div.elementor-posts-container article');
      const results = [];
      const seenUrls = new Set(); // To prevent duplicate URLs if any

      // Iterate over the first 10 articles found
      Array.from(nodes).slice(0, 10).forEach(article => {
        const linkEl = article.querySelector('h2.elementor-post__title a');
        const dateEl = article.querySelector('span.elementor-post-date');

        const title = linkEl ? linkEl.textContent.trim() : null;
        const url = linkEl ? linkEl.getAttribute('href') : null;
        const date = dateEl ? dateEl.textContent.trim() : null;

        // Ensure title and URL exist and URL hasn't been added before
