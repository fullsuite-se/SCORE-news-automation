...
async function esgpostCompaniesScraper() {
  const browser = await puppeteer.launch({ headless: false, defaultViewport: null, slowMo: 50 });
  const page = await browser.newPage();
  const url = 'https://esgpost.com/category/companies/';

  try {
    console.log('Navigating to ESGPost Companies page...');
    ...
    const filename = 'esgpostCompanies.json';
