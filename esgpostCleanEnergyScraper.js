...
async function esgpostCleanEnergyScraper() {
  const browser = await puppeteer.launch({ headless: false, defaultViewport: null, slowMo: 50 });
  const page = await browser.newPage();
  const url = 'https://esgpost.com/category/clean-energy/';

  try {
    console.log('Navigating to ESGPost Clean Energy page...');
    ...
    const filename = 'esgpostCleanEnergy.json';
